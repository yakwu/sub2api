...
func buildErrorTrendByDimQuery(dimension, where, bucketExpr string, limitArgIndex int) (string, error) {
	...
	b.WriteString("WITH topn AS (\n")
	b.WriteString("  SELECT " + dim.keyExpr + " AS k, COUNT(*) AS c\n")
	b.WriteString("  FROM ops_error_logs\n  " + where + "\n    AND " + errFilter + "\n")
	b.WriteString("  GROUP BY 1 ORDER BY c DESC, 1 ASC\n")
	b.WriteString(fmt.Sprintf("  LIMIT $%d\n", limitArgIndex))
	b.WriteString("),\n")
	b.WriteString("labeled AS (\n")
	b.WriteString("  SELECT COALESCE(g.k::text, '') AS key, " + labelSelect + " AS label\n")
	b.WriteString("  FROM topn g " + dim.joinSQL + "\n")
	b.WriteString("),\n")
	...
	b.WriteString("  FROM ops_error_logs\n  " + where + "\n    AND " + errFilter + "\n")
	b.WriteString("  GROUP BY 1, 2\n")
	b.WriteString(")\n")
	b.WriteString("SELECT a.bucket, a.key,\n")
	b.WriteString("       COALESCE(l.label, CASE WHEN a.key = '__others__' THEN '其它' ELSE a.key END) AS label,\n")
	b.WriteString("       a.total, a.sla, a.business_limited\n")
	b.WriteString("FROM agg a LEFT JOIN labeled l ON l.key = a.key\n")
	b.WriteString("ORDER BY a.bucket ASC, a.total DESC")
	return b.String(), nil
}
